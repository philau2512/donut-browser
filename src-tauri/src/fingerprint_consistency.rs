...
fn proxy_url(settings: &crate::browser::ProxySettings) -> Option<String> {
  match settings.proxy_type.to_lowercase().as_str() {
    "http" | "https" | "socks4" | "socks5" => Some(
      crate::proxy::proxy_manager::ProxyManager::build_proxy_url(settings),
    ),
    _ => None,
  }
}
...
fn language_matches_country(cc: &str, language: &str) -> Option<bool> {
  crate::browser::camoufox::geolocation::locale_selector()?.region_speaks(cc, language)
}
...
pub async fn check_profile_consistency(
  ...
  let now = crate::proxy::proxy_manager::now_secs();
  ...
  let (exit_tz, exit_cc, exit_ip) = if let Some(c) = cached {
    ...
    let exit_ip = crate::proxy::ip_utils::fetch_public_ip(Some(&url))
      .await
      .map_err(|e| format!("exit-node lookup failed: {e}"))?;
    match crate::browser::camoufox::geolocation::get_geolocation(&exit_ip) {
      Ok(geo) => {
...
pub async fn match_profile_fingerprint_to_exit(
  ...
  let refreshed = crate::browser::wayfern_manager::WayfernManager::refresh_fingerprint_geolocation(
    &fingerprint,
    None,
    Some(&geoip_override),
  )
