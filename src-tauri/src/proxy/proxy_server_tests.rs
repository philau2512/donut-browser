...
mod tests {
  ...
  async fn test_proxy_storage() {
    ...
    let config =
      crate::proxy::proxy_storage::ProxyConfig::new(id.clone(), "DIRECT".to_string(), Some(8080));

    // Save config
    crate::proxy::proxy_storage::save_proxy_config(&config).unwrap();

    // Load config
    let loaded = crate::proxy::proxy_storage::get_proxy_config(&id).unwrap();
    assert_eq!(loaded.id, id);
    assert_eq!(loaded.upstream_url, "DIRECT");
    assert_eq!(loaded.local_port, Some(8080));

    // Delete config
    assert!(crate::proxy::proxy_storage::delete_proxy_config(&id));
    assert!(crate::proxy::proxy_storage::get_proxy_config(&id).is_none());
  }
