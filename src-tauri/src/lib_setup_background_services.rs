    ...
    Ok(settings) => {
      if settings.mcp_enabled {
        ...
        tauri::async_runtime::spawn(async move {
          match crate::mcp::mcp_server::McpServer::instance()
            .start(mcp_handle)
            .await
          {
...
fn spawn_status_broadcast(_app: &AppHandle) {
  ...
  tauri::async_runtime::spawn(async move {
    ...
    let mut interval = tokio::time::interval(tokio::time::Duration::from_secs(FAST_INTERVAL_SECS));
    ...
    loop {
      ...
      if !any_has_pid && !any_was_running {
        ...
        if current_interval_secs != IDLE_INTERVAL_SECS {
          ...
          interval = tokio::time::interval(tokio::time::Duration::from_secs(IDLE_INTERVAL_SECS));
          interval.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Skip);
        }
        continue;
      }
              ...
              if is_running {
                ...
                let _ = runner
                  .handle_profile_stopped(
                    &app_handle_status,
                    &profile_id,
                    Some("Detected by Status Checker poll"),
                    false,
                  )
                  .await;
    ...
    match crate::settings::settings_manager::get_app_settings(app_handle_api.clone()).await {
      Ok(settings) => {
        if settings.api_enabled {
          ...
          match crate::api::api_server::start_api_server_internal(
            settings.api_port,
            &app_handle_api,
          )
          .await
