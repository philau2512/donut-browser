...
if (files.length === 0) {
  ...
  try {
    execSync("pnpm lint:js && pnpm lint:rust && pnpm lint:spell", {
      stdio: "inherit",
      cwd: rootDir,
    });
  } catch (err) {
  ...
  for (const file of files) {
    ...
    try {
      if (relativePath.endsWith(".rs")) {
        ...
        execSync(`node scripts/cargo-clippy.mjs`, {
          stdio: "inherit",
          cwd: rootDir,
        });
      } else if (
        ...
        if (relativePath.startsWith("donut-sync/")) {
          ...
          execSync(
            `node scripts/biome-run.mjs --cwd donut-sync check ${syncFileRelative}`,
            {
              stdio: "inherit",
              cwd: rootDir,
            },
          ...
          execSync(`node scripts/biome-run.mjs check ${relativePath}`, {
            stdio: "inherit",
            cwd: rootDir,
          });
        }
